 ...
 * ============================================================== */
typedef struct monitored_entry {
  pid_t pid;
  ...
  int soft_limit_exceeded; // 0 = not yet, 1 = already warned
...
static void timer_callback(struct timer_list *t) {
  ...
  list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
    ...
    // Soft limit check
    if (!entry->soft_limit_exceeded && rss > entry->soft_limit_bytes) {
    ...
    if (rss > entry->hard_limit_bytes) {
      kill_process(entry->container_id, entry->pid, entry->hard_limit_bytes,
                   rss);
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
  ...
  if (cmd == MONITOR_REGISTER) {
    printk(KERN_INFO "[container_monitor] Registering container=%s pid=%d "
                     "soft=%lu hard=%lu\n",
           req.container_id, req.pid, req.soft_limit_bytes,
           req.hard_limit_bytes);

    monitored_entry_t *entry;
